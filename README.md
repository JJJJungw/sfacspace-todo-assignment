...
####  바인딩 주소의 의미와 실험 결과
* **localhost (127.0.0.1)**
    * **의미**: '루프백' 주소로, "이 컨테이너 안에서 발생하는 요청만 받겠다"라고 선언하는 상태입니다.
    * **거절 이유**: 호스트 PC의 브라우저는 컨테이너 입장에서 '외부망'.  포트 포워딩을 통해 요청이 컨테이너 대문까지는 도착하지만, 서버 엔진이 "내부 요청만 받겠다"며 요청을 반려합니다.
* **0.0.0.0**
...
* **데이터 유실 **: 컨테이너는 기본적으로 '휘발성'입니다. 컨테이너 내부 쓰기 계층에 저장된 데이터는 컨테이너가 **삭제**되는 순간 함께 사라집니다.
* **Volume의 역할**: 호스트의 특정 폴더와 컨테이너 내부의 데이터 폴더를 동기화하여, 컨테이너가 교체되거나 삭제되어도 실제 데이터는 유지되도록 합니다.
* **학습 포인트**: 스테이트풀한 DB 서비스에서 볼륨 설정은 선택이 아닌 필수임을 깨달았습니다.
...
* **상태 확인 (Liveness/Readiness Probe)**: 운영 환경에서 오케스트레이터가 서버의 생존 여부를 주기적으로 체크합니다.
