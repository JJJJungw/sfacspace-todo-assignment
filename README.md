...
* **localhost (127.0.0.1)**
    ...
    * **거절 이유**: 호스트 PC의 브라우저는 컨테이너 입장에서 **'외부망'**입니다. 포트 포워딩을 통해 요청이 컨테이너 대문까지는 도착하지만, 서버 엔진이 "내부 요청만 받겠다"며 요청을 반려(Connection Refused)합니다.
* **0.0.0.0**
    * **의미**: 서버의 **모든 네트워크 인터페이스**를 개방한다는 의미입니다. 
...
* **원인 분석**:
    1.  **컨테이너는 독립적인 개체**: Docker 환경에서 `localhost`는 호스트 PC가 아닌 **해당 컨테이너 자기 자신**을 의미합니다.
...
* **해결 방법**:
    * Docker Compose가 생성한 내부 가상 네트워크를 통해 서비스 명칭인 **`db`**를 호스트로 사용해야 합니다.
